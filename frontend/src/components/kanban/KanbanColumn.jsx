...
export function KanbanColumn({ column, tasks = [], onSelectTask, onOpenCreateTask }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl bg-slate-100/80 dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 p-3 sm:p-4 min-h-[500px] w-full transition-colors">
      {/* Column Header */}
      <div className="flex items-center justify-between pb-3 mb-2 border-b border-slate-200/80 dark:border-slate-800">
        ...
        {onOpenCreateTask && (
          <button
            ...
            className="p-1 rounded-lg text-slate-400 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-100 hover:bg-white dark:hover:bg-slate-800 transition-colors cursor-pointer"
          ...
          <div
            ...
            className={`flex-1 space-y-3 p-1 rounded-xl transition-colors min-h-[200px] ${
              snapshot.isDraggingOver
                ? 'bg-blue-50/70 dark:bg-blue-950/40 ring-2 ring-blue-500/30 dark:ring-blue-500/40'
                : ''
            }`}
          >
            ...
            {tasks.length === 0 && !snapshot.isDraggingOver && (
              <div className="h-32 flex items-center justify-center border-2 border-dashed border-slate-200/80 dark:border-slate-800 rounded-xl text-xs text-slate-400 dark:text-slate-500 text-center p-4">
