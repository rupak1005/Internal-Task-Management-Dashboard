...
export function AuditLogsPage({ onSelectTask }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      {/* Filter Bar */}
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200/90 dark:border-slate-700 shadow-xs flex items-center gap-2 transition-colors">
      ...
      {loading && logs.length === 0 ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 text-slate-500 dark:text-slate-400 transition-colors">
          No audit records found matching the selected filter.
        </div>
      ) : (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/90 dark:border-slate-700 shadow-xs overflow-hidden divide-y divide-slate-100 dark:divide-slate-700/60 transition-colors">
          {logs.map((log) => (
            <div
              key={log.id}
            ...
            >
              <div className="space-y-1 flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-700/80 text-slate-700 dark:text-slate-300 border border-slate-200/60 dark:border-slate-600/60">
              ...
              <div className="text-[11px] text-slate-400 dark:text-slate-400 shrink-0 flex items-center gap-1 sm:self-start">
